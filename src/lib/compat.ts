 ...
 * page is rendered. This app is used mostly on a phone, so the three methods it
 * relies on are filled in here.
 *
 ...
 * Loaded as a side effect: import this before touching pdf.js, on the main
 * thread *and* inside its worker — a worker has its own globals and inherits
 * nothing from the page. That is what `pdf-worker.ts` is for.
 */
